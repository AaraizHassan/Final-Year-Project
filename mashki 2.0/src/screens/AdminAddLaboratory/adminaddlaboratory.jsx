...
const AdminAddLaboratory = () => {
  ...
  const fetchLabCredentials = async () => {
    try {
      ...
      // 192.168.100.57:5000
      // 192.168.100.10
      const response = await fetch('http://192.168.100.10:5000/admin/lab-credentials');
      if(response.ok){
  ...
  const handleAddLabCredentials = async () => {
    try {
      ...
      const response = await fetch('http://192.168.100.10:5000/admin/add-lab-credentials',{
        method: 'POST',
        headers: {
  ...
  const handleDeleteLabCredentials = async (labPhoneNumber) => {
    try {
      ...
      const response = await fetch(`http://192.168.100.10:5000/admin/delete-lab-credentials/${labPhoneNumber}`, {
        method: 'DELETE',
      });
      if (response.ok) {
