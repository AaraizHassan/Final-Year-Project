...
export const HomeScreen = ({ route }) => {
  ...
  const fetchPrice = async () => {
    try {
      const response = await fetch(`http://192.168.100.10:5000/price/12LitersRefill`);
      if (response.ok) {
      ...
      >
        <View style={styles.menuContainer}>
          ...
          <TouchableOpacity onPress={handleMenuButton2Press}>
            <Text style={styles.menuItem}>Edit Address</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={handleMenuButton3Press}>
            <Text style={styles.menuItem}>Give Feedback</Text>
          </TouchableOpacity>
        </View>
      </Modal>
