...
const AdminPrice = () => {
  ...
  const fetchItemNames = async () => {
    try {
      const response = await fetch('http://192.168.100.10:5000/items');
      if (response.ok) {
  ...
  const fetchCurrentPrice = async (itemName) => {
    try {
      const response = await fetch(`http://192.168.100.10:5000/price/${itemName}`);
      if (response.ok) {
        const data = await response.json();
        return data.price;
      } else {
  ...
  const handleUpdatePrice = async () => {
    try {
      const response = await fetch('http://192.168.100.10:5000/update-price/12LitersRefill', {
        method: 'PUT',
        headers: {
